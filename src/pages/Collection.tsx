...
const Collection : React.FC = () => {
  ...
  if (!context) {
    throw new Error("Collection must be used within a ShopProvider");
  }
  ...
  const toggleCategory = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (category.includes(target.value)) {
      setCategory(prev => prev.filter(item => item !== target.value));
    }
    else {
      setCategory(prev => [...prev, target.value]);
    }
  }

  const toggleSubCategory = (e: React.ChangeEvent<HTMLInputElement>) => {
    const target = e.target as HTMLInputElement;
    if (subCategory.includes(target.value)) {
      setSubCategory(prev => prev.filter(item => item !== target.value));
    }
    else {
      setSubCategory(prev => [...prev, target.value]);
    }
  }
  ...
  return (
    <div className="flex flex-col md:flex-row gap-1 md:gap-10 pt-10 border-t w-[96%] md:w-[93%] mx-auto px-5">
